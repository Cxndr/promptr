			...
			setSpanStates((prevStates) =>
				prevStates.map((span) => {
					...
					const isBeyondXBounds =
						nextX <= buffer || nextX >= window.innerWidth - 20 - buffer; // Approx width minus buffer
					const isBeyondYBounds =
						nextY <= buffer || nextY >= window.innerHeight - 20 - buffer; // Approx height minus buffer
					...
					return {
						...
						pos: {
							x: isBeyondXBounds
								? Math.max(
										buffer,
										Math.min(nextX, window.innerWidth - 20 - buffer)
								  )
								: nextX,
							y: isBeyondYBounds
								? Math.max(
										buffer,
										Math.min(nextY, window.innerHeight - 20 - buffer)
								  )
								: nextY,
						},
						dir: {
